...
public class AppServiceResourceHandler : IResourceDiscovery, IResourceLifecycle, IResourceDiagnostics, IResourceHealth
{
    ...
    public async Task<ResourceHealthResult> GetHealthAsync(ServicePrincipal servicePrincipal, Resource resource, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to fetch health for App Service {ResourceId}", resource.ResourceId);
        }
